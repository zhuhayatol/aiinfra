...
import regex as re
import os
from tiny_lm.tokenizer.gpt4 import GPT4Tokenizer
...
@pytest.mark.parametrize("tokenizer_factory", [BPETokenizer, RegexTokenizer, GPT4Tokenizer])
...
def test_gpt4_tiktoken_equality_special_tokens():
    ...
    assert gpt4_tokenizer_ids == tiktoken_ids
    assert tokenizer.decode(gpt4_tokenizer_ids) == specials_string

# reference test to add more tests in the future
@pytest.mark.parametrize("tokenizer_factory", [BPETokenizer, RegexTokenizer])
